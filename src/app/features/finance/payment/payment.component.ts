...
import { VendorPayment } from '../../../core/models/vendor.model';
  ...
  template: `
    <div class="space-y-5 stagger">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 class="text-2xl font-bold text-white tracking-tight">Payments Received</h1>
          <div class="flex items-center gap-3 mt-1">
            <p class="text-slate-500 text-sm">
              Showing <span class="text-emerald-400 font-semibold">{{ filtered.length }}</span> of {{ payments.length }}
            </p>
          ...
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="border-b border-blue-500/10 bg-slate-900/30">
                <th class="py-3.5 px-6 text-[10.5px] font-bold text-slate-500 uppercase tracking-widest">Document No</th>
            ...
            <tbody>
              <tr *ngFor="let pay of filtered" class="border-b border-blue-500/5 last:border-0 hover:bg-emerald-500/5 transition-colors">
                <td class="py-3.5 px-6 font-mono font-bold text-emerald-400 text-sm">{{ pay.Belnr }}</td>
                <td class="py-3.5 px-6 text-sm text-slate-400">{{ pay.Budat | date:'dd MMM yyyy' }}</td>
                <td class="py-3.5 px-6"><span class="badge-amber">{{ pay.Waers }}</span></td>
                <td class="py-3.5 px-6 text-sm font-mono text-white text-right">{{ pay.Wrbtr | number:'1.2-2' }}</td>
              </tr>
              <tr *ngIf="filtered.length === 0">
                <td colspan="4" class="py-14 text-center">
                  <span class="material-icons text-4xl text-slate-700 block mb-2">search_off</span>
                  <p class="text-slate-500 text-sm">No payments match your filter</p>
...
export class PaymentComponent implements OnInit {
  ...
  ngOnInit() {
    this.svc.getPayments().subscribe({
      next: (data) => { this.payments = data; this.buildMeta(); this.applyFilter(); this.isLoading = false; },
      error: () => {
        this.payments = [
          { Lifnr: '11', Belnr: '2000000001', Budat: '2023-10-06', Wrbtr: 5000.50, Waers: 'INR' },
          { Lifnr: '11', Belnr: '2000000002', Budat: '2023-10-16', Wrbtr: 12500.00, Waers: 'INR' },
        ];
  ...
  buildMeta() {
    this.currencies = [...new Set(this.payments.map(p => p.Waers).filter(Boolean))];
  }

  applyFilter() {
    this.filtered = this.payments.filter(p => {
      const docNo = p.Belnr || '';
      const matchSearch = !this.searchTerm || docNo.toLowerCase().includes(this.searchTerm.toLowerCase());
      const matchCurr = !this.currencyFilter || p.Waers === this.currencyFilter;
      return matchSearch && matchCurr;
    });
    this.filteredTotal = this.filtered.reduce((acc, p) => acc + Number(p.Wrbtr || 0), 0);
  }
